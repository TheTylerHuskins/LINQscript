 ...
 * Standard predicates
 */
export let QueryPredicate = {

  /**
   * Always returns true
   */
  Always: (() => true) as IQueryPredicate<any>,

  /**
   * Always returns false
   */
  Never: (() => false) as IQueryPredicate<any>,

  /**
   * Returns true if !!item is true
   */
  Truethy: ((item) => !!item) as IQueryPredicate<any>,

  /**
   * Returns true if !item is true
   */
  Falsey: ((item) => !item) as IQueryPredicate<any>
};
 ...
 */
export let EqualityComparer = {
  /**
   * Compares the two items using the `===` operator
   */
  Default: ((a: any, b: any) => a === b) as IEqualityComparer<any>
};
...
export interface IQueryable<TSource> {
  ...
  SelectMany<TInner, TResult>(selector: IQuerySelector<TSource, Iterable<TInner>>, resultSelector:
